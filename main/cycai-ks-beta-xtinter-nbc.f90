...
real(8), dimension (2*n,2*m)  :: e,S,agrad,field,ksx,betax
real(8), dimension (n,m)  :: half_ksx,half_betax,half_e, half_S
...
character(len=32) :: parameter_file,ICn_file,ICS_file,FCn_file,FCS_file,n_file,S_file,empty,ksx_file,kst_file,betax_file,betat_file
real(8) :: ks,a_deg,b_deg,ec50_deg,n_deg,K_deg ,kst, betat,kst0, betat0, kst1, betat1
...
call getarg(4,ksx_file)
write(*,*) ksx_file

call getarg(5,kst_file)
write(*,*) kst_file

call getarg(6,betax_file)
write(*,*) betax_file

call getarg(7,betat_file)
write(*,*) betat_file

call getarg(8,n_file)
if (n_file.eq.empty) n_file = 'a.dat'
write(*,*) n_file
call getarg(9,S_file)
if (S_file.eq.empty) S_file = 'i.dat'
write(*,*) S_file

call getarg(10,FCn_file)
if (FCn_file.eq.empty) FCn_file = 'fa.dat'
write(*,*) FCn_file
call getarg(11,FCS_file)
...
open(unit=90,file=ksx_file,status='unknown',form='unformatted')
open(unit=100,file=kst_file,status='unknown',form='unformatted')
open(unit=110,file=betax_file,status='unknown',form='unformatted')  
open(unit=120,file=betat_file,status='unknown',form='unformatted')  
...
do k=1,m ! Mirror param_profiles
  do j=1,n
   ksx(j,k) = half_ksx(j,k)
   ksx(2*n -(j-1),k) = half_ksx(j,k)
   ksx(j,2*m - (k-1)) = half_ksx(j,k)
   ksx(2*n -(j-1),2*m - (k-1)) = half_ksx(j,k)
   betax(j,k) = half_betax(j,k)
   betax(2*n -(j-1),k) = half_betax(j,k)
   betax(j,2*m - (k-1)) = half_betax(j,k)
   betax(2*n -(j-1),2*m - (k-1)) = half_betax(j,k)
  enddo
enddo
...
do idx_write = 1, nint(t_final/t_write)  !Loop over times in which writing is written
  ...
  do i = 1, n_write !Integration between times in which data is written
    kst = kst0 + (i-1)*(kst1 - kst0)/n_write
    betat = betat0 + (i-1)*(betat1 - betat0)/n_write    

    !Compute nonlinear part e
    field = (betax + betat)*(ksx + kst) + (a_cdc25 + b_cdc25*e**n_cdc25/(K_cdc25 + e**n_cdc25))*S &
          - (a_wee1 + b_wee1*K_wee1/(K_wee1 + e**n_wee1))*e  &
          - (betax + betat)*(a_deg + b_deg*e**n_deg/(K_deg + e**n_deg))*e
    ...
    field = - (a_cdc25 + b_cdc25*e**n_cdc25/(K_cdc25 + e**n_cdc25))*S &
          + (a_wee1 + b_wee1*K_wee1/(K_wee1 + e**n_wee1))*e  &
          - (betax + betat)*(a_deg + b_deg*e**n_deg/(K_deg + e**n_deg))*S
    status = DftiComputeForward(punt_f,field_equiv,field_fourier_equiv)    !Transform the nonlinear terms into Fourier space.
